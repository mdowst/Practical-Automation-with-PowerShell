...
foreach ($s in $servers) {
    # Set parameters for the Test-NetConnection calls
    $test = @{
        ComputerName     = $s
    ...
    try {
        # create hashtable for New-PSSession parameters
        $PSSession = @{
            ErrorAction = 'Stop'
        }
        # if listening on WSMan port
        if (Test-NetConnection @test -Port 5985) {
            $PSSession.Add('ComputerName', $s)
        }
        # if listening on SSH port
        elseif (Test-NetConnection @test -Port 22) {
            $PSSession.Add('HostName', $s)
            $PSSession.Add('UserName', $LinuxUser)
        }
...
# Remainder of the script is unchanged from listing 2
$Command = @{
    Session  = $Sessions
    FilePath = $ScriptFile
}
